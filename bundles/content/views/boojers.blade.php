		...
		<div class="row-fluid">
		    <div class="span12 relative" id="boojers-page">
		        ...
		        <div class="boojers-nav clearfix">
		        	<button class="text-right active" data-action="toggle-tag" data-target="#pro"><span>Yeah we're professionals,</span></button>
			        		...
			        		<? if (!empty($boojers)): ?>
					        	<? foreach ($boojers as $boojer): ?>
					        		<li class="boojer-item" data-id="all">
					        			<a href="/boojers/<?=$boojer->username;?>" data-action="get-bio" target="_blank"><img src="<?=$boojer->professional_photo_small;?>" alt="photo of <?=$boojer->first_name;?> <?=$boojer->last_name;?>"></a>
					        			<div>
					        				<h3><a href="/boojers/<?=$boojer->username;?>" data-action="get-bio" target="_blank"><?=$boojer->first_name;?> <?=$boojer->last_name;?></a></h3>
					        				<p><?=$boojer->title;?></p>
					        			</div>
					        		</li>
					        	<? endforeach; ?>
			        ...
			        <div class="boojer-tab" id="fun">
			        	...
			        	<ul class="display-list thumbnails">
			        		<? if (!empty($boojers)): ?>
					        	<? foreach ($boojers as $boojer): ?>
					        		<li class="boojer-item" data-id="all">
					        			<a href="/boojers/<?=$boojer->username;?>" data-action="get-bio" target="_blank"><img src="<?=$boojer->fun_photo_small;?>" alt="photo of <?=$boojer->first_name;?> <?=$boojer->last_name;?>"></a>
					        			<div>
					        				<h3><a href="/boojers/<?=$boojer->username;?>" data-action="get-bio" target="_blank"><?=$boojer->first_name;?> <?=$boojer->last_name;?></a></h3>
					        				<p><?=$boojer->title;?></p>
					        			</div>
...
@section('scripts')
	<script>
		...
		<? if (!empty($boojers)): ?>
		<? foreach ($boojers as $boojer): ?>
		pros.push({'tags': '<?foreach ($boojer->tags as $tag) { if ($tag->type === "professional") echo strtolower(str_replace(' ', '-', $tag->name)) . ',' ;}?>', 'html': '<a href="/boojers/<?=$boojer->username;?>" data-action="get-bio" target="_blank"><img src="<?=$boojer->professional_photo_small;?>" alt="photo of <?=str_replace("'", "\'", $boojer->first_name);?> <?=str_replace("'", "\'", $boojer->last_name);?>"></a><div><h3><a href="/boojers/<?=$boojer->username;?>" data-action="get-bio" target="_blank"><?=str_replace("'", "\'", $boojer->first_name);?> <?=str_replace("'", "\'", $boojer->last_name);?></a></h3><p><?=$boojer->title;?></p></div>'});
		fun.push({'tags': '<?foreach ($boojer->tags as $tag) { if ($tag->type === "fun") echo strtolower(str_replace(' ', '-', $tag->name)) . ',' ;}?>', 'html': '<a href="/boojers/<?=$boojer->username;?>" data-action="get-bio" target="_blank"><img src="<?=$boojer->fun_photo_small;?>" alt="photo of <?=str_replace("'", "\'", $boojer->first_name);?> <?=str_replace("'", "\'", $boojer->last_name);?>"></a><div><h3><a href="/boojers/<?=$boojer->username;?>" data-action="get-bio" target="_blank"><?=str_replace("'", "\'", $boojer->first_name);?> <?=str_replace("'", "\'", $boojer->last_name);?></a></h3><p><?=$boojer->title;?></p></div>'});
		<? endforeach; ?>
		<? endif; ?>
		$(function () {
			pageJS.boojersPage();
